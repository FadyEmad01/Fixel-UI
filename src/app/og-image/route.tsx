...
function getSlugFromRequest(req: Request): string {
  ...
  return slugParam.split("/").filter(Boolean).join("/");
}

export async function GET(req: Request) {
  const slug = getSlugFromRequest(req);
  const block = blocksMetadata.find((b) => b.id === slug);
  const category = blocksCategoriesMetadata.find((c) => c.id === slug);

  if (slug && !block && !category) {
    return notFound();
  }

  const title = slug ? block?.name ?? category?.name ?? "Fixel UI" : "Fixel UI";
  const description = slug
    ? block
      ? `Free shadcn/ui ${category?.name.toLowerCase() ?? "block"} block`
      : `${category?.count ?? 0} free shadcn/ui blocks`
    ...
    <div tw="relative flex w-full h-full bg-[#0A0A0A]">
      ...
      >
        ...
        <div tw="flex flex-row w-full justify-between items-end" style={{ gap: 40 }}>
          ...
          <div tw="flex ml-6">
            <p tw="text-white/80 text-2xl -mb-2" style={{ fontFamily: "Syne" }}>
              {siteConfig.url}
            </p>
          </div>
        </div>
      ...
      fonts: [
        {
          ...
          data: await loadGoogleFont(
            "Syne",
            `${title} ${description} ${siteConfig.url.replace("https://", "")}`,
          ),
          style: "normal",
        },
      ],
