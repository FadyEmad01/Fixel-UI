...
export async function GET(
  ...
  const block = blocksMetadata.find((b) => b.id === slugString);
  const category = blocksCategoriesMetadata.find((c) => c.id === slugString);

  if (!block && !category) notFound();

  const title = block ? block.name : category?.name ?? "Fixel UI";
