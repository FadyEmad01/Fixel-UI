...
export function getAllBlocks() {
  const allBlocksData: BlocksProps[] = [];

  blocksCategoriesMetadata.forEach((categoryMetadata) => {
    const categoryBlocks = getBlocks({ blocksCategory: categoryMetadata.id });
    if (categoryBlocks && categoryBlocks.blocksData) {
      allBlocksData.push(...categoryBlocks.blocksData);
    }
  });
